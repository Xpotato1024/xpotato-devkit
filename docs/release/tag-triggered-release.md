...
On Windows, the archive contains `devkit-installer.exe` only. The installer embeds the `devkit.exe` payload, so it can be redistributed as a single file after extraction.
