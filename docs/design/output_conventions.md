...
- Human-facing standard output may use ANSI color when writing to a compatible terminal.
- `--brief` output must remain single-line plain text with no ANSI escape sequences.
- JSON output must remain uncolored.
- If color support is unavailable or disabled by environment, commands must fall back to plain text without changing content.
