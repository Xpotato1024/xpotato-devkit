...
pub fn split_frontmatter(text: &str) -> (&str, &str) {
    ...
    if let Some(pos) = text[3..].find("\n---") {
        ...
        if let Some(close_pos) = text[abs_pos + 1..].find("---") {
            ...
            while after_close < bytes.len() && (bytes[after_close] == b'\r' || bytes[after_close] == b'\n') {
                after_close += 1;
            }
...
fn heading_level(line: &str) -> usize {
    ...
    if !stripped.starts_with("#") { return 0; }
...
fn find_section(lines: &[&str], heading: &str, exact: bool) -> Result<(usize, usize, usize), String> {
    ...
    if matches.len() > 1 {
        let locs: Vec<_> = matches.iter().map(|(idx, _)| format!("L{}", idx + 1)).collect();
        return Err(format!("Heading '{}' matched {} sections ({}). Provide a more specific heading.", heading, matches.len(), locs.join(", ")));
    }
...
pub fn append_to_section(filepath: &Path, heading: &str, mut content: String, dry_run: bool) -> Result<String, String> {
    let text = fs::read_to_string(filepath).map_err(|e| e.to_string())?;
    let (frontmatter, body) = split_frontmatter(&text);
    let lines = get_lines(body);
    let plain_lines: Vec<_> = lines.iter().map(|l| l.trim_end_matches('\n').trim_end_matches('\r')).collect();
    ...
    for line in &lines[..end_idx] { result.push_str(line); }
...
pub fn replace_section(filepath: &Path, heading: &str, mut content: String, keep_heading: bool, dry_run: bool) -> Result<String, String> {
    let text = fs::read_to_string(filepath).map_err(|e| e.to_string())?;
    let (frontmatter, body) = split_frontmatter(&text);
    let lines = get_lines(body);
    let plain_lines: Vec<_> = lines.iter().map(|l| l.trim_end_matches('\n').trim_end_matches('\r')).collect();
    ...
    let mut result = String::from(frontmatter);
    let to_idx = if keep_heading { content_start } else { start_idx };
    for line in &lines[..to_idx] { result.push_str(line); }
    result.push_str(&content);
    for line in &lines[end_idx..] { result.push_str(line); }
...
pub fn ensure_section(filepath: &Path, heading: &str, mut content: String, level: usize, after: Option<&str>, dry_run: bool) -> Result<String, String> {
    let text = fs::read_to_string(filepath).map_err(|e| e.to_string())?;
    let (frontmatter, body) = split_frontmatter(&text);
    let lines = get_lines(body);
    let plain_lines: Vec<_> = lines.iter().map(|l| l.trim_end_matches('\n').trim_end_matches('\r')).collect();
    ...
    if let Some(after_heading) = after {
        ...
        for line in &lines { result.push_str(line); }
...
pub fn append_bullet(filepath: &Path, heading: &str, bullet: &str, dedupe: bool, dry_run: bool) -> Result<String, String> {
    let text = fs::read_to_string(filepath).map_err(|e| e.to_string())?;
    let (frontmatter, body) = split_frontmatter(&text);
    let lines = get_lines(body);
    let plain_lines: Vec<_> = lines.iter().map(|l| l.trim_end_matches('\n').trim_end_matches('\r')).collect();
